...
rss = true
tags = ["linux", "ricing", "customization", "guide", "wayland", "x11"]
+++
...
> [!IMPORTANT]
>  I'm covering here is from personal experience and personal opinions, feel free to add anything you find useful about this discussion in the comments below!
...
Both of them are important for an desktop graphical workflow, but they have different purposes.
...
Most DEs are composed of a **[Window Manager](#window-managers)** (more on that later), Icons, specific folder structure, widgets and toolbars.
...
The protocols in which the clients and the server communicate are very important, because they're what defines which DE or WM can be used, since they implement an client of an specific server that implements a specific protocol.

The architecture of the server changes depends on which protocol is implemented.

The most used protocols are [X11](https://en.wikipedia.org/wiki/X_Window_System_core_protocol) and [Wayland](https://wayland.freedesktop.org/).

### X11

The most implemented and used X11 server is the [Xorg](https://www.x.org/wiki/) server, most of the DEs and WMs support it and its very compatible with a variety of applications.
...
* [Xcompmgr](https://gitlab.freedesktop.org/xorg/app/xcompmgr/)
...
This defines your system's :sparkles:*looks*:sparkles:. Basically, it's your system's colors, harmony and stylistic choices. This covers from window colors, appearance, mouse styles and icons.

Programs that use the **GTK Toolkit** can be customized with **GTK Themes**, and programs using **QT** can be customized using **QT** themes. If you have programs that use both and want them to look uniform, read [this guide](https://wiki.archlinux.org/title/Uniform_look_for_Qt_and_GTK_applications), it explains tips and techniques for uniformizing the two different toolkits.

### UI Styling

This one's tricky, because it depends on which DE or WM you chose. It's the customizations you can do on your DE/WM's layout and interface by interacting with their configuration files.
In some DEs this can be configured in the settings panel, on others you have to change configuration files or even code new implementations on the DE/WM's code.
...
Take a look in the [r/unixporn](https://www.reddit.com/r/unixporn/) subreddit and get some inspiration for your workflow, plan and test a lot of different configurations until you find one that you feel confortable with, and then, customize it piece by piece, like the ship of theseus, it will eventually become your own configuration.
...
Versioning your config files *(namely dotfiles)* is the best way to reuse and rollback your configuration if anything happens. That way you can also share your files with other Linux users if you want to! Just be sure to not commit any sensitive information.
...
[Dotfiles](https://github.com/savio-henrique/nixos-config)
...
![Majora Laptop 1](wm-majora-1.png)
![Majora Laptop 2](wm-majora-2.jpeg)
![Majora Laptop 3](wm-majora-3.png)

## Desktop

![Chrono Desktop 1](wm-chrono-1.jpeg)
